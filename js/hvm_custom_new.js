...
if(/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent) 
	...
	jq$(document).ready(function() 
	{
	  ...
	  //   	jq$(document).bind({
			 ...
			 jq$(document).find("select[name='High School']").on("change",function(e) 
	    	{
	    		...
	    		{
	    			if(jq$(document).find("select[name='High School']").val() == "Others")
					{
	    		...
	    		}else if(jq$(document).find("select[name='Highest Qualification']").val() == "GCE A-Level (Local)")
	    		{
	    			if(jq$(document).find("select[name='High School']").val() == "Others")
					{
						jq$(document).find("input[name='Other GCE School']").val('');
						jq$(document).find("input[name='Other GCE School']").prev().show();
				 		jq$(document).find("input[name='Other GCE School']").show();
					}else
					{
						jq$(document).find("input[name='Other GCE School']").val('NA');
						jq$(document).find("input[name='Other GCE School']").prev().hide();
				 		jq$(document).find("input[name='Other GCE School']").hide();
					}
	    		}
	    ...
	    jq$(document).find("select[name='Highest Qualification']").on("change",function(e) 
	    {
				...
				jq$(document).find("input[name='Other IB School']").attr('placeholder','Type here..');
				jq$(document).find("input[name='Other GCE School']").attr('placeholder','Type here..');
				...
				if(jq$(document).find("select[name='Highest Qualification']").val() == "GCE A-Level (Local)")
				{
					...
					var includeOther=true;	
					addNeededOptionsBacktoList(1,22,includeOther);
					jq$(document).find("select[name='High School']").prop('selectedIndex',0);
     ...
     // 				jq$(document).find("input[name='IB']").prev().show();
					...
					var includeOther=true;
					addNeededOptionsBacktoList(27,48,includeOther);	
	...
	//>>>>>tooltip logic
	    ...
	    function showTooltip1(event) 
	    {
		   ...
		   var tooltipX = jq$(event.currentTarget).offset().left-5;
...
//selectPushArray
	...
	function addNeededOptionsBacktoList(lowerIndex,upperIndex,other)
	{
		removeAllOptions();
		...
		if(other==true)
		{
			jq$(jq$(document).find("select[name='High School']")).append(selectPushArray[48]);
		}
	}
